...
| **Audit Controls** — record & examine activity (R) | §164.312(b) | ✅ | Append-only, **keyed (HMAC-SHA256) hash-chained** `AuditLog`: login/MFA (incl. failures), list, search, view, create, update. The MAC key lives in app config, not the DB, so DB-only write access cannot recompute the chain to hide tampering. `app/audit.py` | Every action produces a row; tampering breaks the chain, reported by `/api/audit/verify` |

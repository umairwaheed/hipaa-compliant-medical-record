...
pytestmark = pytest.mark.skipif(
    not TEST_DB, reason="set TEST_DATABASE_URL to run integration tests"
)
...
def _enroll_mfa(username: str, password: str) -> str:
    ...
    secret = client.post("/api/auth/mfa/enroll", headers={"Authorization": f"Bearer {pre}"}).json()[
        "secret"
    ]
    r = client.post(
...
def test_successful_mfa_resets_counter_for_legit_user():
    secret = (
        _create_user("gooduser", "Password-1234!"),
        _enroll_mfa("gooduser", "Password-1234!"),
    )[1]
